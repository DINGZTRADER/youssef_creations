...
export default function Storefront(){
  ...
  const suggestions=useMemo(()=>{
    ...
    const ranked=products.filter(p=>p.price<=max).filter(p=>{
      ...
      if(o.includes('casual')||o.includes('date')||o.includes('event')||o.includes('party')) return p.category==='Complete Looks'||p.category==='Trousers';
      return true;
    });
  ...
  const itemMessage=p=>`Hello YousefCreationz, I want this item:\n\n${p.name}\n${p.category}\n${money(p.price)}\n\nPlease confirm availability, size/colour options and delivery in Kampala.`;
  const orderBag=()=>{
  ...
  return <main>
    ...
    <section className="hero" id="top">
      ...
      <div className="hero-image-wrap"><img src={`${STORE.imageOrigin}/products/congo-look.jpeg`} alt="YousefCreationz featured look"/><div className="hero-sticker"><span>Latest</span><span>Drop</span></div><div className="hero-caption">Tap a product, then order on WhatsApp <span>→</span></div></div>
    </section>
    ...
    <section className="collection-section" id="collection">
      ...
      <div className="shop-tools"><label className="search-box"><span>Search</span><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search products"/></label><p>{shown.length} item{shown.length===1?'':'s'}</p></div>
      ...
      <div className="product-grid">{shown.map((p,i)=><article className={`product-card product-${(i%5)+1}`} key={p.id}><button className="product-image-wrap" onClick={()=>setSelected(p)}><img src={p.image} alt={p.name}/>{p.badge&&<span className="product-badge">{p.badge}</span>}<span className="quick-add">View details <b>+</b></span></button><div className="product-info"><div><p className="product-category">{p.category}</p><h3>{p.name}</h3></div><p className="product-price">{money(p.price)}</p></div><div className="card-actions"><a className="mini-wa" href={wa(itemMessage(p))} target="_blank" rel="noreferrer">Order on WhatsApp</a><button onClick={()=>shareProduct(p)} aria-label={`Share ${p.name}`}>Share</button></div></article>)}</div>
    ...
    <section className="whatsapp-section"><div><p className="eyebrow">Fastest way to order</p><h2>Message the shop directly.</h2><p>{STORE.whatsappDisplay} • Kampala, Uganda</p></div><a className="whatsapp-button" href={wa('Hello YousefCreationz, I am browsing your website and would like some help.')} target="_blank" rel="noreferrer">Chat on WhatsApp ↗</a></section>

    <footer><a className="brand footer-brand" href="#top"><span className="brand-mark">YC</span> Yousef<span>Creationz</span></a><p>Clothing and style essentials, ordered the way Kampala already shops.</p><span className="footer-actions"><a href="/admin">Shop Manager ↗</a><a href={wa('Hello YousefCreationz, I would like to make an enquiry.')} target="_blank" rel="noreferrer">WhatsApp the shop ↗</a></span></footer>
    ...
    {selected&&<div className="modal-backdrop" onClick={()=>setSelected(null)}><section className="product-modal" onClick={e=>e.stopPropagation()}><button className="close" onClick={()=>setSelected(null)}>×</button><img src={selected.image} alt={selected.name}/><div><p className="eyebrow">{selected.category}</p><h2>{selected.name}</h2><p className="big-price">{money(selected.price)}</p><p>Availability, sizes and exact colour options are confirmed on WhatsApp before payment.</p><div className="stack"><a className="button whatsapp-primary" target="_blank" rel="noreferrer" href={wa(itemMessage(selected))}>Order this on WhatsApp</a><button className="button button-dark" onClick={()=>{add(selected);setSelected(null)}}>Add to bag</button><button className="button button-outline" onClick={()=>shareProduct(selected)}>Share item</button></div></div></section></div>}
    ...
    <dialog id="bag" className="bag-dialog"><form method="dialog"><button className="close">×</button></form><p className="eyebrow">Your bag</p><h2>{bag.length?`${bag.length} item${bag.length>1?'s':''}`:'Your bag is empty'}</h2>{bag.map((p,i)=><div className="bag-line" key={`${p.id}-${i}`}><span>{p.name}<button className="remove-item" onClick={()=>remove(i)}>Remove</button></span><b>{money(p.price)}</b></div>)}{bag.length>0&&<><div className="bag-total"><span>Total</span><b>{money(total)}</b></div><button className="button whatsapp-primary full" onClick={orderBag}>Send order to WhatsApp</button></>}</dialog>
  </main>;
}
